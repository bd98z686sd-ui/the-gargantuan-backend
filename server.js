...
app.post('/api/generate-video', requireAdmin, async (req, res) => {
  ...
  (async () => {
    try {
      ...
      // Use a slightly lower resolution for the generated video to speed up
      // ffmpeg processing on limited Render dynos.  Reducing the dimensions
      // from 854x480 to 640x360 roughly halves the pixel count and results
      // in faster encoding while maintaining a 16:9 aspect ratio.
      const spectrumFilter = 'showspectrum=s=640x360:mode=combined:legend=disabled';
      try {
        await runFfmpegWithFilter(spectrumFilter);
      } catch (err) {
        console.warn('Spectrum filter failed, falling back to wave filter', err.message);
        const waveFilter = 'showwaves=s=640x360:mode=line:rate=24,format=yuv420p';
