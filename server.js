  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ffmpeg(localAudio)
        ...
        .complexFilter([
          '[0:a]aformat=channel_layouts=stereo,showspectrum=s=854x480:mode=combined:legend=disabled[v]'
        ])
      ...
      return await new Promise((resolve, reject) => {
        ffmpeg(localAudio)
          .outputOptions(['-y', '-threads', '1', '-preset', 'ultrafast', '-r', '24'])
          .complexFilter([
            '[0:a]aformat=channel_layouts=stereo,showwaves=s=854x480:mode=line:rate=24,format=yuv420p[v]'
          ])
          .outputOptions(['-map', '[v]', '-map', '0:a', '-shortest', '-pix_fmt', 'yuv420p', '-movflags', '+faststart'])
          .videoCodec('libx264')
          .audioCodec('aac')
